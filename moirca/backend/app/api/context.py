...
from .chat import AGENT_NAMES, AGENT_PROMPTS, is_placeholder_api_key
...
def _build_prompt(req: ContextAskRequest) -> Tuple[str, bool]:
    ...
    head = f"页面标题: {req.page_title}\n页面地址: {req.page_url}\n" if (req.page_title or req.page_url) else ""
    body = f"\n\n--- 页面内容(片段) ---\n{ctx}" if ctx else ""

    prompt = (
        f"用户在浏览一个高考志愿相关的网页,并选中/关注了下面这段内容。\n"
        f"{head}"
        f"{body}\n\n"
        f"用户的问题是:\n{req.question}\n\n"
