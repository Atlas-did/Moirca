...
def _sanitize_history(history: List[dict]) -> List[dict]:
    """过滤客户端传入的 history，仅保留 role ∈ {user, assistant} 的消息。

    防止客户端通过 {"role":"system", ...} 注入系统层指令。
    同时把 content 强制为字符串，杜绝非文本载荷。
    """
    cleaned: list[dict] = []
...
def _build_llm_messages(agent_id: str, message: str, history: List[dict]) -> List[dict]:
    system = AGENT_PROMPTS.get(agent_id, AGENT_PROMPTS["master"])
    messages: list[dict] = [{"role": "system", "content": system}]
    messages.extend(_sanitize_history(history)[-6:])
...
def _get_llm_response(agent_id: str, message: str, history: List[dict]) -> str:
    ...
    try:
        ...
        if is_placeholder_api_key(Config.LLM_API_KEY):
            return ""
        client = LLMClient()
        messages = _build_llm_messages(agent_id, message, history)
...
def chat_stream(request: ChatRequest):
    """SSE 流式 Agent 对话 — 真实逐 token 推送（LLM stream=True），无 LLM 时整体返回降级语料。"""
    ...
    def generate():
        # 无可用 LLM → 降级语料整体返回一次，绝不伪造打字机效果
        from ..config import Config
        if is_placeholder_api_key(Config.LLM_API_KEY):
            yield _sse({"type": "meta", "agent_name": agent_name_map.get(request.agent_id, request.agent_id), "model_used": "fallback"})
            yield _sse({"type": "chunk", "content": _get_fallback_reply(request.agent_id)})
            yield "data: [DONE]\n\n"
            return
        ...
        try:
            from ..utils.llm_client import LLMClient
            client = LLMClient()
            messages = _build_llm_messages(request.agent_id, request.message, request.history)
            stream = client.chat_stream(messages, temperature=0.8, max_tokens=300)
        except Exception:
            stream = None
        ...
        yield _sse({"type": "meta", "agent_name": agent_name_map.get(request.agent_id, request.agent_id), "model_used": "llm"})
        got_any = False
        try:
            for delta in stream:
                if delta:
